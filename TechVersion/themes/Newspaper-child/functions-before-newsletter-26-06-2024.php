...
function newsletter_weekly_func() {
    ...
    //$query = "SELECT * FROM `{$wpdb->prefix}posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-5 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "' AND post_type IN ('post') AND post_status = 'publish' ORDER BY `{$wpdb->prefix}posts`.`post_date` ASC LIMIT 0, 1";
    ...
    if (!empty($result)) {
        ...
        $para = $paragraph;
        $email_content .= '<p>' . $para . '</p>
                                                    <p><a class="link-btn" style="background-color: #04a353;color: #FFF;font-size: 11px !important;line-height: 35px !important;letter-spacing: 1px !important;height: auto;border-radius: 3px;padding: 0 16px;box-sizing: border-box;text-decoration: none;display: inline-block;" href="' . get_the_permalink($result->ID) . '"> Read More </a></p>
                                                </td>
                                            </tr>
                                        </table>';
    }
   ...
   //$query = "SELECT * FROM `{$wpdb->prefix}posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-5 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "' AND post_type IN ('post') AND post_status = 'publish' ORDER BY `{$wpdb->prefix}posts`.`post_date` ASC LIMIT 1, 2";
   ...
   $query = "SELECT * FROM `{$wpdb->prefix}posts` WHERE post_type IN ('post') AND post_status = 'publish' ORDER BY `{$wpdb->prefix}posts`.`post_date` ASC LIMIT 1, 2";
    ...
    if (!empty($results)) {
        ...
        //                                 <tr><td><p><a class="link-btn" style="text-decoration: none; font-size: 13px !important;color:#FFF;margin-top:20px;padding:5px 30px;background-color : #04a353;line-height : 35px !important ;letter-spacing : 1px !important ;height : auto;border-radius : 3px;box-sizing : border-box;text-decoration : none;display : inline-block;" href="' . site_url('news') . '">  Click here for more news  </a></p></td></tr>
        //                             </table>';
    }
	...
	$query = "SELECT * FROM `{$wpdb->prefix}posts` WHERE post_type IN ('e_books','infographics','white_papers') AND post_status = 'publish' ORDER BY `{$wpdb->prefix}posts`.`post_date` ASC LIMIT 1, 3";
    $results = $wpdb->get_results($query, OBJECT);
    ...
    // if (count($results) == 1) {
    //     $query = "SELECT * FROM `{$wpdb->prefix}posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-2 week')) . "' AND post_type IN ('e_books','infographics','white_papers') AND post_status = 'publish' ORDER BY `{$wpdb->prefix}posts`.`post_date` ASC LIMIT 1, 3";
    //     $results = $wpdb->get_results($query, OBJECT);
    // }
