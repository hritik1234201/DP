...
if (have_posts()) {
    ...
    font-style: italic;
    color: #aaa;float:right;margin-top:-20px;font-size:12px;position: relative;
 z-index: 9999;"><?php 
echo do_shortcode('[image_courtesy_shortcode]') ?></div>
                ...
                <span class="td-post-date td-post-source" style="margin-top: 5px;color:#000">
            <b>News Source:</b> <a href="<?php echo (get_post_meta($post->ID, '_source_url', true) != '') ? get_post_meta($post->ID, '_source_url', true) : home_url(); ?>" target="_blank"><?php echo (get_post_meta($post->ID, '_source_name', true) != '') ? get_post_meta($post->ID, '_source_name', true) : 'source'; ?></a></span>
            </div>
