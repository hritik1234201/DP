                    ...
                    <body style="-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;max-width:720px;margin: 0 auto;font-size: 15px;line-height: 28px;color: #333333;">
                        ...
                        <div lang="en" style="" role="article" aria-label="An email from Your Brand Name">
                            ...
                            <div role=article aria-label="An email from Your Brand Name" lang=en class="main-div" style="font-weight:400;margin:0 auto;padding:0 20px 40px 20px;color: #333333;">
                                ...
                                <!-- {{{email_content}}} -->
                                <?php
                                $query = "SELECT * FROM `wp_posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-1 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "' AND post_type = 'post' AND post_status = 'publish' ORDER BY `wp_posts`.`post_date` DESC LIMIT 0, 1";
...
<tr>
                                ...
                                //Second Row
                                $query = "SELECT * FROM `wp_posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-1 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "' AND post_type = 'post' AND post_status = 'publish' ORDER BY `wp_posts`.`post_date` DESC LIMIT 1, 4";
...
//                                        $str = substr($str, 0, strpos($str, '</p>') + 6);
                                ...
                                $query = "SELECT * FROM `wp_posts` WHERE post_date >= '" . date('Y-m-d', strtotime('-1 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "' AND post_type IN ('resources') AND post_status = 'publish' ORDER BY `wp_posts`.`post_date` DESC LIMIT 1, 3";
                                $results = $wpdb->get_results($query, OBJECT);
                                $email_content .= '<table class="single-row-news" border="0" cellspacing="0" cellpadding="0" style="width: 100%;margin-bottom: 20px;">
                                        <tr style="height: 140px; vertical-align: top;">';
                                $i = 0;
...
//                                </table>';
                                ...
                                <?php
//                                $email_content = '';
                                $query = "SELECT * FROM `wp_posts` WHERE post_type = 'post' AND post_date >= '" . date('Y-m-d', strtotime('-1 week')) . "' AND post_date < '" . date('Y-m-d', strtotime('-1 day')) . "'  AND post_status = 'publish' ORDER BY `wp_posts`.`post_date` DESC LIMIT 0, 3";
                                $results = $wpdb->get_results($query, OBJECT);
                                foreach ($results as $result) {
