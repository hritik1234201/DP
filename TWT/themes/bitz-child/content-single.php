...
<article itemtype="http://schema.org/Article" itemscope="" id="post-<?php the_ID(); ?>" <?php post_class('single-layout clearfix'); ?> >
    ...
    <?php if (get_post_meta(get_the_ID(), 'single_post_header', true) != 'off') : ?>
        <header class="entry-header clearfix">
            ...
            <?php
            if (get_post_meta(get_the_ID(), 'post_lead_text', true) != '') {
    ...
    <?php
    if (get_post_meta(get_the_ID(), 'content_featured_img', true) != 'off' && has_post_thumbnail()) {
        echo '<div class="post-preview">', the_post_thumbnail('large') . '</div>';
    }
    ?>
     <div style="margin-bottom: 15px;
    font-style: italic;
    color: #aaa;float:right;margin-top:-20px;font-size:12px;color:"><?php 
echo do_shortcode('[image_courtesy_shortcode]') ?></div>
    <?php
    if (get_post_meta(get_the_ID(), 'mnky_featured_image_caption_text', true) != '' && has_post_thumbnail()) {
    ...
    <?php } ?>
        ...
        <?php
        if (get_post_meta(get_the_ID(), 'top_post_advertisement', true) != '') {
    ...
    <div class="td-post-date td-post-source" style="margin-top: 5px;">
        <?php if(get_post_meta($post->ID, '_source_url', true) != '') { ?>
        News Source: <a href="<?php echo (get_post_meta($post->ID, '_source_url', true) != '') ? get_post_meta($post->ID, '_source_url', true) : home_url(); ?>" target="_blank"><?php echo (get_post_meta($post->ID, '_source_name', true) != '') ? get_post_meta($post->ID, '_source_name', true) : 'source'; ?></a>
        <?php } ?>
    </div>
    ...
    <?php
    if (get_post_meta(get_the_ID(), 'bottom_post_advertisement', true) != '') {
